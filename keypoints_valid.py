...
def mean_coords(keypoints, names):
    points = [keypoints[name] for name in names if valid(keypoints[name])]
    if not points:
...
# MARIOLA HO HE CANVIAT, PERQ VOLIA FER UNA INDIVIDUAL, DESCOMENTA LA LINEA D'ABAIX, COMENTA LA Q VAIG POSAR COM A SUBSITUCIÓ 
# I COMENTA LA DE FOLDER=PATH(FOLDER)
subfolders = [f for f in baby_folder.iterdir() if f.is_dir()] #DESCOMENTA AIXO
# subfolders = ["/home/cvmsct05/temperatures_max_min/44/20.11.24-21.11.24"] #COMENTA AIXO
for folder in subfolders:
    # folder = Path(folder) ##### COMENTA AIXO
    out = Path(folder)
    print(out)
    # if out == Path("/home/cvmsct05/temperatures_max_min/29/202408 1"): # empty folder, created by hand
    #     continue
